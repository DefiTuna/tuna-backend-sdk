...
import { getStakingPosition, getStakingPositionHistory, setTunaBaseUrl, StakingPositionActionType } from "../src";
...
describe("Staking position", async () => {
  const response = await getStakingPosition(TEST_WALLET_ADDRESS);
...
describe("Staking position history", async () => {
  const response = await getStakingPositionHistory(TEST_WALLET_ADDRESS);
  const data = response.data!.data!;
