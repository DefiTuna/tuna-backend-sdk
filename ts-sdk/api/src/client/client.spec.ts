...
describe("LP positions history", async () => {
  ...
  const positionWithoutLeverage = tunaPositions.find(v => v.positionAddress === "6SaKKYAAddvbMoqpoUyrDTkTv9qxifVpTcip539LFNjs");
  const positionWithLeverage = tunaPositions.find(v => v.positionAddress === "AiNPCv5iqPxXCfmfn7ySGQ6mBRKMN3pM8wXNmm6VPbEq");
  ...
  it("Test position 1 variables", () => {
    ...
    expect(positionWithoutLeverage?.pool).toBe(SOL_USDC_POOL_ADDRESS);
    ...
    expect(positionWithoutLeverage?.entryPrice).toBe(0.006760562464516597);
  ...
  it("Test position 2 variables", () => {
    ...
    expect(positionWithLeverage?.pool).toBe(SOL_USDC_POOL_ADDRESS);
    ...
    expect(positionWithLeverage?.entryPrice).toBe(0.006754327193235526);
...
describe("LP positions history query options", async () => {
  ...
  it("AfterPosition options", async () => {
    const positions = await client.getUserLpPositions(TEST_WALLET_ADDRESS, { afterPosition: "AiNPCv5iqPxXCfmfn7ySGQ6mBRKMN3pM8wXNmm6VPbEq" });
    expect(positions.length).toBe(1);
  });

  it("OpenedAt options", async () => {
    const positions = await client.getUserLpPositions(TEST_WALLET_ADDRESS, { openedAt: { from: new Date("2025-03-07T19:32:20Z"), to: new Date("2025-03-07T19:32:20Z")} });
    expect(positions.length).toBe(1);
  });

  it("ClosedAt options", async () => {
    const positions = await client.getUserLpPositions(TEST_WALLET_ADDRESS, { closedAt: { from: new Date("2025-03-07T19:32:20Z"), to: new Date("2025-03-07T19:32:20Z")} });
    expect(positions.length).toBe(0);
  });
});

describe("LP position actions", async () => {
  const actions = await client.getUserLpPositionActions(TEST_WALLET_ADDRESS, "6SaKKYAAddvbMoqpoUyrDTkTv9qxifVpTcip539LFNjs");
  ...
  it("OpenPosition parameters", () => {
    ...
    expect(openPosition.txSignature).toBe("2qgLvLArPc3qKod4MHbeE2ri2PAA9nrCV3ubqvGhVQ6TfxQNFhCn5tLiheLbk4UiPqzr2XTfCKQve3mdT3adQheG");
  ...
  it("IncreaseLiquidity parameters", () => {
    ...
    expect(increaseLiquidity.txSignature).toBe("2qgLvLArPc3qKod4MHbeE2ri2PAA9nrCV3ubqvGhVQ6TfxQNFhCn5tLiheLbk4UiPqzr2XTfCKQve3mdT3adQheG");
